...
bool testIntCopyTo() {
    {
        ...
        test.enqueue(1);
        test.enqueue(2);
        intPtr* values = new intPtr[test.count()];
        test.copyTo(values);
...
bool testValueCopyTo() {
    {
        ...
        test.enqueue(Value(1));
        test.enqueue(Value(2));
        ValuePtr* values = new ValuePtr[test.count()];
        test.copyTo(values);
        if(!(*values[0] == 1 && *values[1] == 2)) {
            delete[] values;
            return false;
...
// Do not support on web, so the mutex does not worked.
void lockValueAction(Values *test) {
    Locker locker(test);
    Thread::msleep(500);
    test->clear();
}
...
bool testValueLock() {
    ...
    Thread thread("test lock thread", lockValueAction, &test);
    thread.start();
    if (test[1] != 2) {
        return false;
    }
