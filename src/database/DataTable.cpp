...
namespace Database {
    DataColumn::DataColumn(const String &name, const String &type, bool pkey) : DataColumn(name, convertType(type),
                                                                                           pkey) {
    }
    ...
    DataCell *DataCells::at(size_t pos) const {
        return PList<DataCell>::at(pos);
    }
    ...
    const String
    DataTable::columnNameStr(bool hasTableName, const StringArray *excludedNames, const char *splitStr) const {
        String sb;
