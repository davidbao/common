...
namespace Database
{
	...
	const String OracleClient::getErrorMsg()
	{
	...
	uint OracleClient::executeSqlInner(const String& sql)
	{
		...
		result = OCIStmtPrepare(statement, _oracleDb->error, (const OraText*)sql.c_str(), (ub4)sql.length(), OCI_NTV_SYNTAX, OCI_DEFAULT);
		if (result)
		{
			...
			table_columnCount == 0)
		{
			...
			for (int i = 0; i < numcols; i++)
			{
				...
				result = OCIAttrGet((dvoid*)paramhp, (ub4)OCI_DTYPE_PARAM,
					(dvoid**)&col_name, (ub4 *)&col_name_len, (ub4)OCI_ATTR_NAME, (OCIError *)_oracleDb->error);
				result = OCIAttrGet((dvoid*)paramhp, (ub4)OCI_DTYPE_PARAM,
				...
				{
					result = OCIAttrGet((dvoid*)paramhp, (ub4)OCI_DTYPE_PARAM,
						(dvoid**)&datasize, 0, (ub4)OCI_ATTR_DATA_SIZE, (OCIError *)_oracleDb->error);
				}
			...
			for (int i = 0; i < numcols; i++)
			{
				...
				if (!(icolumn->datatype == SQLT_BLOB || icolumn->datatype == SQLT_CLOB))
				{
					result = OCIDefineByPos(statement, &define, _oracleDb->error, i + 1,
						(dvoid *)icolumn->datavalue.strValue, icolumn->datasize, SQLT_STR, nullptr, nullptr, nullptr, OCI_DEFAULT);
					if (result)
					{
						OCIHandleFree(statement, OCI_HTYPE_STMT);
	...
	uint OracleClient::executeSqlInsertInner(const DataTable& table)
	{
		...
		for (uint j = 0; j < columnCount; j++)
		{
			...
			result = OCIBindByPos(statement, &binds[j], _oracleDb->error, j + 1, (dvoid *)values[0], StrLength, SQLT_CHR, 0, alenp, 0, 0, 0, OCI_DEFAULT);
			if (result)
			{
		...
		result = OCIStmtExecute(_oracleDb->context, statement, _oracleDb->error, rowCount, 0, nullptr, nullptr, OCI_BATCH_ERRORS);
