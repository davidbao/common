...
namespace rpc
{
    RpcClientContext::RpcClientContext(const Endpoint& endpoint, const Secure& secure) : endpoint(endpoint), secure(secure)
    {
    }
    ...
    RpcStatus RpcClient::invoke(const RpcMethodContext& context, const IRpcSyncRequestData& request, IRpcSyncResponseData& response)
    {
        if(_service == nullptr)
            initService();
    ...
    RpcStatus RpcClient::invokeAsync(const RpcMethodContext& context, const IRpcAsyncRequestData& request, const IRpcAsyncResponseData& response, async_callback action, void* owner)
    {
        if(_service == nullptr)
            initService();
    ...
    bool RpcClient::sendSync(const RpcMethodContext& context, const RpcSyncRequest& request, RpcSyncResponse& response, const String& name)
    {
        return _service->sendSync<RpcSyncRequest, RpcSyncResponse, RpcSyncContext>(request, response, name, context.tryCount);
    }
    ...
    void RpcClient::generateInstructions(void* owner, Instructions* instructions)
    {
        ...
        instructions->add(new RpcAsyncRequestInstruction(new InstructionDescription("RpcAsyncRequest", new RpcAsyncRequestContext())));
        instructions->add(new ServerRpcAsyncResponseInstruction(new InstructionDescription("ServerRpcAsyncResponse", new RpcAsyncResponseContext()), cs));
        ...
        instructions->add(new ServerRpcSyncInstruction2(new InstructionDescription("ServerRpcSync", new RpcSyncContext()), cs));
        
        instructions->add(new ServerRpcAsyncRequestInstruction2(new InstructionDescription("ServerRpcAsyncRequest", new RpcAsyncRequestContext()), cs));
        instructions->add(new RpcAsyncResponseInstruction2(new InstructionDescription("RpcAsyncResponse", new RpcAsyncResponseContext())));
        
        instructions->add(new ServerRpcNotifyInstruction2(new InstructionDescription("ServerRpcNotify", new RpcNotifyContext()), cs));
    }
