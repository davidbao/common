...
namespace rpc
{
    RpcAsyncElement::RpcAsyncElement(const Uuid& token, const IRpcAsyncRequestData* request, const IRpcAsyncResponseData* response, async_callback action, void* owner)
    {
        this->token = token;
    ...
    RpcSyncMethod::RpcSyncMethod(const String& name, const IRpcSyncRequestData& request,
                                 const IRpcSyncResponseData& response, sync_callback action, void* owner) : RpcMethod(name, request)
    {
        this->response = response.clone();
        this->action = action;
        this->owner = owner;
    ...
    RpcAsyncMethod::RpcAsyncMethod(const String& name, const IRpcAsyncRequestData& request,
                                 const IRpcAsyncResponseData& response, async_callback action, void* owner) : RpcMethod(name, request)
    {
        this->response = response.clone();
        this->action = action;
        this->owner = owner;
    ...
    RpcStatus RpcSender::invoke(const RpcMethodContext& context, const IRpcSyncRequestData& request, IRpcSyncResponseData& response)
    {
        if(connected())
        {
...
//            bool result = _service->sendSync<RpcSyncRequest, RpcSyncResponse, RpcSyncContext>(rrequest, rresponse, "RpcSync", context.tryCount);
    ...
    RpcStatus RpcSender::invokeAsync(const RpcMethodContext& context, const IRpcAsyncRequestData& request, const IRpcAsyncResponseData& response, async_callback action, void* owner)
    {
        if(connected())
        {
            RpcAsyncRequest rrequest(context, &request);
            _asyncElementsMutex.lock();
            _asyncElements.add(rrequest.token(), new RpcAsyncElement(rrequest.token(), &request, &response, action, owner));
...
//            bool result = _service->sendAsync<RpcNotifyInfo, RpcNotifyContext>(rinfo, "RpcNotify");
    ...
    bool RpcReceiver::onSyncSetValue(const RpcMethodContext& context, Stream* stream, IRpcSyncRequestData* request, IRpcSyncResponseData*& response)
    {
        RpcMethod* m = nullptr;
    ...
    bool RpcReceiver::onAsyncRequestSetValue(const RpcMethodContext& context, Stream* stream, const Uuid& token, const Endpoint& peerEndpoint)
    {
        RpcMethod* m = nullptr;
