...
namespace Common
{
    ...
    MqttClient::MqttClient(const String& clientId) : _client(nullptr), _deliveredtoken(0), _clientId(clientId), _connecting(false), _errorCode(MQTTCLIENT_SUCCESS)
    {
    ...
    bool MqttClient::connect(const ConnectOptions& options)
    {
        ...
        {
            _clientMutex.unlock();
            Trace::error(String::format("Failed to connect mqtt broker'%s', address is incorrect.", _connectOptions.address.c_str()));
            _connecting = false;
            return false;
        }
        _clientMutex.unlock();
        ...
        if ((rc = MQTTClient_connect(_client, &conn_opts)) != MQTTCLIENT_SUCCESS)
        {
            _errorCode = rc;
            _clientMutex.unlock();
            Trace::error(String::format("Failed to connect mqtt broker'%s', return code %d.", _connectOptions.address.c_str(), rc));
            _connecting = false;
            return false;
        }
        _clientMutex.unlock();
    ...
    bool MqttClient::publishMessage(const String& topic, const String& payload, Qos qos, const TimeSpan& timeout)
    {
        ...
        if(rc == MQTTCLIENT_SUCCESS)
        {
            if(timeout != TimeSpan::Zero)
            {
                ...
                {
                    _errorCode = rc;
                    Debug::writeLine(String::format("MqttClient::publishMessage.waitForCompletion failed, error code: %d", rc));
                }
            }
        }
        else
        ...
        if(!connecting() && !connected())
        {
            ...
            if(_errorHandle.errorCount >= _errorHandle.detectionCount)
            {
                ...
                Trace::info(String::format("Mqtt client'%s' is reconnecting! user name: '%s', password: '%s'",
                                           options.address.c_str(), options.userName.c_str(), options.password.c_str()));
