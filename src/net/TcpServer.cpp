        ...
        while (true) {
            ...
            if (ret < 0) {
                ...
                switch (error) {
                    ...
                    default:
                        Trace::error(
                                String::format("Could not accept SSL session'%d': SSL_get_error: %d, errno: %s(%d)",
                                               client->socketId(), error, strerror(errno), errno));
                        SSL_CTX_free(ctx);
                        SSL_free(ssl);
                        return false;
                }
                ...
                if (TickTimeout::isTimeout(startTime, timeout)) {
                    Trace::error(String::format("Could not accept SSL session'%d': timeout! SSL_get_error: %d",
                                                client->socketId(), error));
                    SSL_CTX_free(ctx);
                    SSL_free(ssl);
                    return false;
                }
...
//        }
    ...
    bool WebSocketServer::handshaking(TcpClient *client, const TimeSpan &timeout) {
        ...
        if (length == (int) header.length()) {
            ...
            length = client->Receiver::receiveByEndBytes(key, sizeof(key), start, sizeof(start) - 1, end,
                                                         sizeof(end) - 1, 0, 3000);
            if (length > 0) {
