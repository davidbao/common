...
//            Debug::writeFormatLine("opcua connected state: %d", state);
    ...
    bool OpcuaClient::read(const OpcuaNodeId* nodeId, T& value, int type)
    {
        ...
        if(connected())
        {
            ...
            UA_StatusCode status = nodeId->isString() ?
                                    UA_Client_readValueAttribute(_client, UA_NODEID_STRING(nodeId->nsIndex, (char*)nodeId->string.c_str()), &v) :
                                    UA_Client_readValueAttribute(_client, UA_NODEID_NUMERIC(nodeId->nsIndex, nodeId->numeric), &v);
            if(status == UA_STATUSCODE_GOOD &&
    ...
    bool OpcuaClient::write(const OpcuaNodeId* nodeId, T value, int type)
    {
        ...
        if(connected())
        {
            ...
            if(nodeId->isString()) {
                status = UA_Client_writeValueAttribute(_client, UA_NODEID_STRING(nodeId->nsIndex, (char*)nodeId->string.c_str()), &v);
            } else if(nodeId->isNumeric()) {
                status = UA_Client_writeValueAttribute(_client, UA_NODEID_NUMERIC(nodeId->nsIndex, nodeId->numeric), &v);
            }
            if(status == UA_STATUSCODE_GOOD) {
                result = true;
            }
    ...
    bool OpcuaClient::subscribe(const OpcuaNodeId* nodeId)
    {
        if(connected())
        {
            ...
            {
                Debug::writeFormatLine("Failed to create subscription, result: %d.", response.responseHeader.serviceResult);
            ...
            if(nodeId->type == OpcuaNodeId::NumericType) {
                ...
                monRequest = UA_MonitoredItemCreateRequest_default(UA_NODEID_STRING(nodeId->nsIndex, (char*)nodeId->string.c_str()));
            } else {
            ...
            UA_MonitoredItemCreateResult monResponse = UA_Client_MonitoredItems_createDataChange(_client, subId,
                                                      ...
                                                      monRequest, s, valueChanged, NULL);
            if(monResponse.statusCode == UA_STATUSCODE_GOOD)
            {
                Debug::writeFormatLine("Monitoring '%s', id %u", nodeId->toString().c_str(), monResponse.monitoredItemId);
            }
            else
            {
                delete s;
            ...
            {
                _subscriptionTimer = new Timer("OpcUA.Subscription", subscriptionCallback, this, _connectOptions.samplingInterval.totalMilliseconds());
    ...
    bool OpcuaClient::subscribes(const NodeIds& nodeIds)
    {
        ...
        if(connected())
        {
            ...
            for(uint32_t i=0;i<count;i++)
            {
                ...
                if(nodeId->type == OpcuaNodeId::NumericType) {
                    items[i] = UA_MonitoredItemCreateRequest_default(UA_NODEID_NUMERIC(nodeId->nsIndex, nodeId->numeric));
                } else if(nodeId->type == OpcuaNodeId::StringType) {
                    items[i] = UA_MonitoredItemCreateRequest_default(UA_NODEID_STRING(nodeId->nsIndex, (char*)nodeId->string.c_str()));
                }
            ...
            UA_CreateMonitoredItemsResponse monResponse = UA_Client_MonitoredItems_createDataChanges(_client, createRequest, (void**)contexts, callbacks, deleteCallbacks);
            ...
            {
                Trace::error(String::format("Monitoring request count'%d', response count'%d'.", count, monResponse.resultsSize));
    ...
    void OpcuaClient::valueChanged(UA_Client *client, UA_UInt32 subId, void *subContext, UA_UInt32 monId, void *monContext, UA_DataValue *value)
    {
        String result;
        ...
        if(response.responseHeader.serviceResult == UA_STATUSCODE_GOOD && response.resultsSize == count) {
            for(uint32_t i=0;i<response.resultsSize;i++) {
                ...
                if(bresult[i].statusCode == UA_STATUSCODE_GOOD && bresult[i].targetsSize == 1) {
                    ...
                    Debug::writeFormatLine("opcua.translate, target error, index: %d, status code: 0x%x, target size: %d",
