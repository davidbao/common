        ...
        if(_connected)
        {
            ...
            else
            {
                Debug::writeFormatLine("connect function'%s' failed with error code: %d, error: %s",
                                       !_peerEndpoint.isEmpty() ? _peerEndpoint.toString().c_str() : ((Int32)_socket).toString().c_str(),
            ...
            if(ret <= 0)
            {
                ...
                switch (error)
                {
                    ...
                    case SSL_ERROR_WANT_WRITE:
                        break;
                    default:
                        Trace::error(String::format("Could not build SSL session: SSL_get_error: %d, errno: %s(%d)", error, strerror(errno), errno));
                        _connected = false;
                        SSL_CTX_free(ctx);
                        SSL_free(ssl);
                        return false;
                ...
                {
                    Trace::error(String::format("Failed to connect SSL server: error: %d, errno: %s(%d)", error, strerror(errno), errno));
                    _connected = false;
                    SSL_free(ssl);
                    SSL_CTX_free(ctx);
                    return false;
...
//            Trace::writeFormatLine("web socket available: %d", count);
    ...
    bool WebSocketClient::readFrame()
    {
        ...
        if (wsHeaderLength != headerLength)
        {
            ...
            Debug::writeFormatLine("web socket'%d' receive error! header length is incorrect! expected length: %d, recv: '%s'",
...
#ifdef DEBUG
            ByteArray messages(original.data(), Math::min((int)original.count(), 128));
            Debug::writeFormatLine("web socket'%d' receive error! header2 length is incorrect! expected length: %d, recv: '%s'",
...
#ifdef DEBUG
            ByteArray messages(original.data(), Math::min((int)original.count(), 128));
            Debug::writeFormatLine("failed to decode socket'%d' original buffer: '%s'", _socket, messages.toString().c_str());
#endif
            return false;
        }
...
//        }
    ...
    WebSocketSSLClient::WebSocketSSLClient(SSLVersion version, int sockfd, bool ipv4, const TimeSpan& receiveTimeout) : TcpSSLClient(version, sockfd, ipv4)
    {
        _position = -1;
        _decoding = true;
        _encoding = true;
...
#ifdef DEBUG
            ByteArray messages(original.data(), Math::min((int)original.count(), 128));
            Debug::writeFormatLine("web ssl socket'%d' receive error! header length is incorrect! expected length: %d, recv: '%s'",
                                   _socket, wsHeaderLength, messages.toString().c_str());
#endif
            return false;
        }
...
#ifdef DEBUG
            ByteArray messages(original.data(), Math::min((int)original.count(), 128));
            Debug::writeFormatLine("web ssl socket'%d' receive error! header2 length is incorrect! expected length: %d, recv: '%s'",
                                   _socket, header2Length, messages.toString().c_str());
#endif
            return false;
        }
...
#ifdef DEBUG
            ByteArray messages(original.data(), Math::min((int)original.count(), 128));
            Debug::writeFormatLine("web ssl socket'%d receive error: length is incorrect! expected length: %d, recv: %s",
                                   _socket, payloadLength, messages.toString().c_str());
#endif
            return false;
        }
...
//        Debug::writeFormatLine("original buffer: '%s', decoded buffer: '%s'", original.toString().c_str(), _buffer.toString().c_str());
    ...
    WebSocketSSLClient::FrameType WebSocketSSLClient::getFrameLength(const ByteArray& inFrame, uint64_t& payloadLength)
    {
        int frameType = WS_OPENING_FRAME;
        const uint8_t *frameData = inFrame.data();
        const size_t frameLength = inFrame.count();
