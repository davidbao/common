...
namespace Communication
{
    ...
    ServerService::ServerService(const Server& server) : BaseCommService(), _server(server), _tsi(nullptr), _usi(nullptr)
    {
    }
    ...
    void ServerService::addDynamicInstructions(const Endpoint& peerEndpoint, Instructions* sendInstructions, Instructions* recvInstructions)
    {
        if(_tsi != nullptr)
        {
    ...
    void ServerService::createUdpSendDevice(const InstructionCallback& callback, const String& ipAddress)
    {
        ...
        ChannelDescription* cd = new ChannelDescription(String::convert("UdpClient_%s", ipAddress.c_str()), "UdpInteractive");
        DeviceDescription* dd = new DeviceDescription(String::convert("UdpClientDevice_%s", ipAddress.c_str()), cd, set);
...
//            Trace::writeLine(mess.c_str(), Trace::Error);
    ...
    void SSLServerService::createDevice(const InstructionCallback& callback)
    {
        ...
        if(secure.enabled)
        {
            ...
            tc->setCertFile(Path::isPathRooted(secure.certFile) ? secure.certFile : Path::combine(path, secure.certFile));
            tc->setCacertFile(Path::isPathRooted(secure.cacertFile) ? secure.cacertFile : Path::combine(path, secure.cacertFile));
        }
    }
