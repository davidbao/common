...
namespace Communication {
    ...
    InstructionContext *
    ClientInstruction::execute(Interactive *interactive, Device *device, InstructionContext *context,
    ...
    InstructionContext *
    ClientInstruction::executeAsync(Interactive *interactive, Device *device, InstructionContext *context) {
        return executeAsync(interactive, device, context, nullptr);
    }

    InstructionContext *
    ClientInstruction::send(Interactive *interactive, Device *device, InstructionContext *context, uint8_t frameId) {
...
//            channel->clearReceiveBuffer();
    ...
    InstructionContext *
    ClientInstruction::receive(Interactive *interactive, Device *device, InstructionContext *context,
                               const ByteArray *buffer) {
        if (interactive == nullptr) {
            return nullptr;
        }
    ...
    InstructionContext *
    ClientInstruction::receiveFromBuffer(Interactive *interactive, Device *device, InstructionContext *context) {
        if (interactive == nullptr) {
            return nullptr;
        }
        ...
        if (hasBackgroudReceiver(interactive)) {
            ...
            if (ci != nullptr) {
                ...
                String info = String::format(
                        "ClientInstruction::receiveFromBuffer, device name: %s, instruction name: %s",
...
//                    Debug::writeFormatLine("ClientInstruction::receiveFromBuffer.timeout, instruction name: %s, timeout: %d ms", description()->name().c_str(), timeout);
    ...
    InstructionContext *
    ClientInstruction::executeSync(Interactive *interactive, Device *device, InstructionContext *context,
                                   const ByteArray *buffer) {
        if (buffer == nullptr) {
...
#ifdef DEBUG
            String info = String::convert("The frame id is not same between sender and receiver, current frame id: %x",
                                          _currentFrameId);
...
//#endif
    ...
    bool FileInstructionEntry::readFile(Stream *stream, const FileHeader *header, uint32_t packetNo,
    ...
    uint32_t FileInstructionEntry::calcPacketCount(FileHeader *header, uint32_t packetLength) {
        ...
        if (!header->path.isNullOrEmpty() && !header->file_name.isNullOrEmpty()) {
            if (File::exists(fileName)) {
                ...
                return (fileLength % packetLength == 0) ? (uint32_t) (fileLength / packetLength) : (uint32_t) (
                        fileLength / packetLength + 1);
            } else {
