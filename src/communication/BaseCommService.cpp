...
namespace Communication {
    ...
    BaseCommService::BaseLoopSender::BaseLoopSender(BaseCommService *service, const String &name,
                                                    const TimeSpan &interval, const Endpoint &endpoint, int sendCount) {
        assert(service);
        _service = service;
        _interval = interval;
        _name = name;
    ...
    void BaseCommService::BaseLoopSender::start() {
        ...
        auto timerProc = [](BaseLoopSender *sender) {
            sender->invoke();
        };
        _timer = new Timer("BaseLoopSenderTimer", _interval, timerProc, this);
    }
    ...
    BaseCommService::BasePacketSender::BasePacketSender(BaseCommService *service, const String &name,
    ...
    void BaseCommService::BasePacketSender::start() {
        assert(_timer == nullptr);
        auto timerProc = [](BasePacketSender *sender) {
            sender->invoke();
        };
        _timer = new Timer("BasePacketSenderTimer", _interval, timerProc, this);
    }
