...
namespace Threading {
    ...
    Timer::Timer(const String &name, timer_callback callback, void *state, int period) : Timer(name, callback, state,
                                                                                               Zero, period) {
    }
    ...
    Timer::Timer(const String &name, timer_callback callback, void *state, const TimeSpan &dueTime,
                 const TimeSpan &period)
            : Timer(name, callback, state, (int) dueTime.totalMilliseconds(), (int) period.totalMilliseconds()) {
    }

    Timer::Timer(const String &name, timer_callback callback, void *state, const TimeSpan &period) : Timer(name,
                                                                                                           callback,
                                                                                                           state,
                                                                                                           TimeSpan::Zero,
                                                                                                           period) {
    ...
    Timer::Timer(const String &name, timer_callback callback, const TimeSpan &period) : Timer(name, callback, nullptr,
                                                                                              period) {
    }
