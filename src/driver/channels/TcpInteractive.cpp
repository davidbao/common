...
namespace Drivers
{
    TcpMultiPlexingReceiver::Client::Client(TcpClient* client, TcpBackgroundReceiver* receiver) : _client(client), _receiver(receiver)
    {
    }
...
#ifdef DEBUG
        String message = String::convert("connect a host, address = %s, port = %d", tcc->address().c_str(), tcc->port());
        Stopwatch sw(message, 1000);
#endif
        _tcpClient = createClient();
        bool result = _tcpClient->connectToHost(tcc->address().c_str(), tcc->port(), tcc->openTimeout(), tcc->reuseAddress());
        if (result)
        {
