...
namespace Drivers
{
	...
	bool SerialInteractive::open()
	{
		...
		String message = String::convert("open a serial port, name = %s, baudrate = %d, databits = %s, parity = %s, stopbit = %s",
                                         scc->portName().c_str(),
                                         scc->baudRate(),
	...
	ssize_t SerialInteractive::send(const uint8_t* buffer, off_t offset, size_t count)
	{
		...
		if(connected())
		{
			len = _port->write((const char*)(buffer+offset), count);
		}
		return len;
	}
	...
	ssize_t SerialInteractive::receive(uint8_t* buffer, off_t offset, size_t count, uint timeout)
	{
#ifdef DEBUG
