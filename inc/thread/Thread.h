...
namespace Threading {
    ...
    public:
        ...
        void startProc(action_callback procAction, uint32_t msec = 1, action_callback startAction = nullptr,
                       action_callback stopAction = nullptr);

        void startProc(action_callback procAction, TimeSpan interval = TimeSpan::fromMilliseconds(1),
                       action_callback startAction = nullptr, action_callback stopAction = nullptr);

        void start(action_callback2 startAction, void *startParam, action_callback2 stopAction = nullptr,
                   void *stopParam = nullptr);

        void startProc(action_callback2 procAction, void *procParam, uint32_t msec = 1,
                       action_callback2 startAction = nullptr,
                       void *startParam = nullptr, action_callback2 stopAction = nullptr, void *stopParam = nullptr);

        void startProc(action_callback2 procAction, void *procParam, TimeSpan interval = TimeSpan::fromMilliseconds(1),
                       action_callback2 startAction = nullptr,
                       void *startParam = nullptr, action_callback2 stopAction = nullptr, void *stopParam = nullptr);

        void start(action_callback3 startAction, ThreadHolder *startParam, action_callback3 stopAction = nullptr,
                   ThreadHolder *stopParam = nullptr);

        void startProc(action_callback3 procAction, ThreadHolder *procParam, uint32_t msec = 1,
                       action_callback3 startAction = nullptr,
                       ThreadHolder *startParam = nullptr, action_callback3 stopAction = nullptr,
                       ThreadHolder *stopParam = nullptr);

        void startProc(action_callback3 procAction, ThreadHolder *procParam,
                       TimeSpan interval = TimeSpan::fromMilliseconds(1), action_callback3 startAction = nullptr,
                       ThreadHolder *startParam = nullptr, action_callback3 stopAction = nullptr,
                       ThreadHolder *stopParam = nullptr);
        ...
        void startProc(const ThreadCallback<T> &procAction, const TimeSpan &interval,
                       const ThreadCallback<T> &startAction = ThreadCallback<T>(),
                       const ThreadCallback<T> &stopAction = ThreadCallback<T>()) {
            if (_autoDelete) {
                if (_tcs.get() == nullptr) {
                    _tcs.reset(this);
                }
    ...
    public:
        ...
        static void
        beginInvoke(action_callback3 startAction, ThreadHolder *startParam, action_callback3 stopAction = nullptr,
                    ThreadHolder *stopParam = nullptr);
