...
namespace Common
{
	...
	public:
        ...
        static bool sdelay(uint32_t sec = 10, delay_callback condition = nullptr, void* parameter = nullptr, uint32_t sleepms = 1);
        static bool msdelay(uint32_t msec = 3000, delay_callback condition = nullptr, void* parameter = nullptr, uint32_t sleepms = 1);
        static bool delay(const TimeSpan& timeout, delay_callback condition = nullptr, void* parameter = nullptr, uint32_t sleepms = 1);
        ...
        template <class T>
        static bool msdelay(uint32_t msec, const DelayCallback<T>& callback = DelayCallback<T>(), uint32_t sleepms = 1)
        {
            return delay(TimeSpan::fromMilliseconds(msec), callback, sleepms);
        }
        template <class T>
        static bool delay(const TimeSpan& timeout, const DelayCallback<T>& callback = DelayCallback<T>(), uint32_t sleepms = 1)
        {
