...
namespace Common
{
	...
	public:
        ...
        Timer(const String& name, timer_callback callback, void* state, const TimeSpan& dueTime, const TimeSpan& period);
        ...
        template <class T>
        Timer(const String& name, const TimerCallback<T>& callback, const TimeSpan& dueTime, const TimeSpan& period) : Timer(name, timerCallback, this, dueTime, period)
        {
            _execution = callback.clone();
        }
        template <class T>
        Timer(const String& name, const TimerCallback<T>& callback, const TimeSpan& period) : Timer(name, callback, TimeSpan::Zero, period)
        {
        }
        template <class T>
        Timer(const String& name, const TimerCallback<T>& callback, int period) : Timer(name, callback, TimeSpan::fromMilliseconds(period))
        {
        }
        template <class T>
        Timer(const String& name, const TimerCallback<T>& callback, int dueTime, int period) : Timer(name, callback, TimeSpan::fromMilliseconds(dueTime), TimeSpan::fromMilliseconds(period))
        {
        }
