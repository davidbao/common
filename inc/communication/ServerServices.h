...
namespace Communication
{
    ...
    public:
        ...
        void updateConfig(const BaseCommConfig& config, const String& prefix, const TypeStrings& typeStrings = TypeStrings::All);
        void updateConfig(const Common::StringMap& properties);
        ...
        // start async sender.
        template<class T, class K, class C>
        void startPacketSender(const String& name, const TimeSpan& interval, int packetCount = 10, const Endpoint& peerEndpoint = Endpoint::Empty)
        {
            if(peerEndpoint.isEmpty())
            {
                PList<ServerService> services;
        ...
        // start sync sender.
        template<class T, class K, class C>
        void startPacketSyncSender(typename ServerService::PacketSyncSender<T, K, C>::FailedAction failedAction, void* owner, const String& name, const TimeSpan& interval, int packetCount = 10, const Endpoint& peerEndpoint = Endpoint::Empty)
        {
            if(peerEndpoint.isEmpty())
            {
                PList<ServerService> services;
        ...
        // start sync sender.
        template<class T, class K, class C>
        void startPacketSyncSender(const String& name, const TimeSpan& interval, int packetCount = 10, const Endpoint& peerEndpoint = Endpoint::Empty)
        {
            if(peerEndpoint.isEmpty())
            {
                PList<ServerService> services;
        ...
        // T is input data, C is context.
        template<class T, class C>
        bool uploadFileSync(const Endpoint& endpoint, const FileHeader& header, const T& inputData, const String& name) const
        {
            ServerService* ss = getService(endpoint);
            if(ss != nullptr)
            {
