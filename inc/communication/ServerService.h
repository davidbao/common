...
namespace Communication {
    ...
    public:
        ...
        public:
            LoopSender(BaseCommService *service, const String &name, const TimeSpan &interval, const Endpoint &endpoint,
                       int sendCount, const T &inputData) : BaseLoopSender(service, name, interval, endpoint,
                                                                           sendCount) {
        ...
        // start async sender.
        template<class T, class C>
        void startLoopSender(const T &inputData, const String &name, const TimeSpan &interval,
        ...
        public:
            ...
            PacketSender(ServerService *cs, const String &name, const TimeSpan &interval, int packetCount = 10)
                    : BasePacketSender(cs, name, interval), _includedBuffer(true), _excludedBuffer(true) {
...
//                            }
        ...
        public:
            ...
            PacketSyncSender(BaseCommService *service, const String &name, const TimeSpan &interval,
                             FailedAction failedAction, void *owner, int packetCount = 10) : BasePacketSender(service,
                                                                                                              name,
                                                                                                              interval),
                                                                                             _includedBuffer(true),
                                                                                             _excludedBuffer(true) {
                _packetCount = packetCount;
            ...
            void processExcluded() {
                ...
                for (uint32_t i = 0; i < buffers.count(); i++) {
                    ...
                    if (count > 0) {
                        ...
                        T data;
                        data.addRange((const K **) datas, count);
                        StatusContext status = ss->sendVectorSyncWithoutClient<T, C>(peerEndpoint, data, _packetCount,
                                                                                     _name);
                        if (!status.isSuccessful() && _failedAction != nullptr)
                            _failedAction(_owner, data);
                        delete[] datas;
...
//#endif
        ...
        // T is input data, K is output data, C is context.
        template<class T, class K, class C>
        bool sendSyncWithoutClient(const Endpoint &endpoint, const T &inputData, K &outputData, const String &name,
                                   int trySendCount = -1) const {
            Endpoints endpoints;
            endpoints.add(endpoint);
        ...
        // T is input data, K is output data, C is context.
        template<class T, class K, class C>
        bool sendSyncWithoutClient(const Endpoints &endpoints, const T &inputData, K &outputData, const String &name,
                                   int trySendCount = -1) const {
            bool result = true;
            InstructionPools ips;
            getClientPoolsWithoutClient(endpoints, ips);
...
//#endif
        ...
        template<class T, class C>
        StatusContext
        sendSync(const Endpoint &endpoint, const T &inputData, const String &name, int trySendCount = -1) const {
            Endpoints endpoints;
            endpoints.add(endpoint);
        ...
        template<class T, class C>
        StatusContext
        sendSync(const Endpoints &endpoints, const T &inputData, const String &name, int trySendCount = -1) const {
        ...
        // sync send input data.
        template<class T, class C>
        StatusContext sendSyncWithoutClient(const Endpoint &endpoint, const T &inputData, const String &name,
                                            int trySendCount = -1) const {
            Endpoints endpoints;
            endpoints.add(endpoint);
        ...
        // sync send input data.
        template<class T, class C>
        StatusContext sendSyncWithoutClient(const Endpoints &endpoints, const T &inputData, const String &name,
                                            int trySendCount = -1) const {
            StatusContext status;
            if (sendSyncWithoutClient<T, StatusContext, ElementSContext<T>>(endpoints, inputData, status, name,
                                                                            trySendCount)) {
                return status;
            }
            return StatusContext(StatusContext::CommunicationError);
...
//                Debug::writeFormatLine("sendVectorSyncWithoutClient.sendAsync, name: %s, count: %d", name.c_str(), temp.count());
        ...
        // T is input data, C is context.
        template<class T, class C>
        bool uploadFileSync(const FileHeader &header, const T &inputData, const String &name,
                            uint32_t packetLength = (uint32_t) -1, const TimeSpan &interval = TimeSpan::Zero) const {
            return uploadFileSync<T, C>(Endpoint::Empty, header, inputData, name, packetLength, interval);
        }

        // T is input data, C is context.
        template<class T, class C>
        bool uploadFileSync(const Endpoint &endpoint, const FileHeader &header, const T &inputData, const String &name,
                            uint32_t packetLength = (uint32_t) -1, const TimeSpan &interval = TimeSpan::Zero) const {
#ifdef DEBUG
            ...
            if (result) {
                String lengthStr;
                if (length < 1024) {
                    lengthStr = UInt32(length).toString();
                ...
                String info =
                        (String) "upload a file %s, file name: %s, file length: %s, speed: %.1f K/s, " + elapsedStr;
        ...
        // T is input data, C is context.
        template<class T, class C>
        void uploadFileAsync(const FileHeader &header, const T &inputData, const String &name,
                             uint32_t packetLength = (uint32_t) -1) const {
            return uploadFileAsync<T, C>(Endpoint::Empty, header, inputData, name, packetLength);
        }

        // T is input data, C is context.
        template<class T, class C>
        void uploadFileAsync(const Endpoint &endpoint, const FileHeader &header, const T &inputData, const String &name,
                             uint32_t packetLength = (uint32_t) -1) const {
#ifdef DEBUG
...
//                Debug::writeFormatLine("packet no: %d", i);
        ...
        // T is input data, C is context.
        template<class T, class C>
        void uploadFileStatusAsync(const Endpoint &endpoint, uint8_t status, const T &inputData, const String &name,
...
//                Debug::writeFormatLine("packet no: %d", i);
    ...
    private:
        ...
        bool sendSync(InstructionPool *ip, const T &inputData, K &outputData, const String &name,
                      int trySendCount = -1) const {
#ifdef DEBUG
            Stopwatch sw(String::convert("ServerService::sendSync, device name: %s, instruction name: %s",
                                         ip->deviceName().c_str(), name.c_str()), 200);
#endif
            if (trySendCount <= 0)
                trySendCount = 1;
