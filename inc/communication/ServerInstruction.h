...
namespace Communication
{
    ...
    protected:
        InstructionContext* executeSync(Interactive* interactive, Device* device, InstructionContext* context, const ByteArray* buffer) override;
    };
    ...
    {
    public:
        ServerUploadPacketEInstruction(InstructionDescription* id, bool syncSendReceive = true) : ServerUploadPacketInstruction<T, EmptyContext>(id, syncSendReceive)
        {
        }
            ...
            if (rcontext != nullptr)
            {
                ...
                if(transfer == BasePacketContext::TransferHeader)
                {
                    ...
                    if(rcontext->status == FileContext<T>::Succeed)
                    {
                        ...
                        {
                            Trace::writeFormatLine("Incoming file download, name: %s", rcontext->header()->file_name.c_str());
                            rcontext->setPacketCount(packetCount);
                            ms.writeUInt32(packetCount);
