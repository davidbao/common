...
namespace Communication {
    ...
    public:
        ...
        public:
            LoopSender(BaseCommService *service, const String &name, const TimeSpan &interval, int sendCount,
                       const T &inputData) : BaseLoopSender(service, name, interval, Endpoint::Empty, sendCount) {
        ...
        public:
            ...
            PacketSender(BaseCommService *service, const String &name, const TimeSpan &interval, int packetCount = 10)
                    : BasePacketSender(service, name, interval), _buffer(MaxBufferLength) {
...
//                    Trace::debug(String::format("send sync packet, buffer count: %d, packet count: %d, name: %s", count, _packetCount, _name.c_str()));
        ...
        // start sync sender.
        template<class T, class K, class C>
        bool startPacketSyncSender(typename PacketSyncSender<T, K, C>::FailedAction failedAction, void *owner,
...
//                Debug::writeFormatLine("sendVectorAsync.sendAsync, name: %s, count: %d", name.c_str(), temp.count());
        ...
        template<class T, class C>
        DownloadFileResult
        downloadFileSync(const String &path, const T &inputData, const String &name, String &fullFileName,
                         FileCommEntry *entry = nullptr, const TimeSpan &timeout = TimeSpan::Zero) const {
#ifdef DEBUG
            ...
            if (isSendSuccessfully(ids->name(), rcontext)) {
                if (rcontext->isFileNotFound()) {
                    ...
                    if (packetCount > 0) {
                        ...
                        String info = (String) "download a file %s, file name: %s, file length: %s, speed: %.1f K/s, " +
                                      elapsedStr;
                        Trace::writeFormatLine(info.c_str(),
                                               result == DownloadFileResult::Succeed ? "successfully"
                                                                                     : "unsuccessfully",
                                               fileName.c_str(),
                                               lengthStr.c_str(),
        ...
        template<class T, class C>
        DownloadFileResult
        downloadFileSync(const String &path, const T &inputData, const String &name, FileCommEntry *entry = nullptr,
                         const TimeSpan &timeout = TimeSpan::Zero) const {
            String fullFileName;
        ...
        template<class T, class C>
        bool
        downloadFileHeaderAsync(const T &inputData, const String &name, uint32_t packetLength = 1 * 1024 * 1024) const {
#ifdef DEBUG
        ...
        // T is input data, C is context.
        template<class T, class C>
        bool downloadFileDataAsync(const T &inputData, const String &name, const FileHeader *header, uint32_t packetNo,
        ...
        template<class T, class C>
        bool
        uploadFileSync(const FileHeader &header, const T &inputData, const String &name, FileCommEntry *entry = nullptr,
